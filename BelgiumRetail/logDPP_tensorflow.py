...
trainingData = pd.read_csv(path+"trainingDataNegSampling.csv")
testData = pd.read_csv(path+"testDataWithTarget.csv")
trainingData['itemSet'] = list(map(literal_eval,trainingData['itemSet']))
testData['itemSet'] = list(map(literal_eval,testData['itemSet']))

#testData_sample = testDataWithTarget.loc[testDataWithTarget.index[:1000]]

nItem = np.max([np.max(list(map(lambda x: np.max(x),trainingData['itemSet']))),
               np.max(list(map(lambda x: np.max(x),testData['itemSet'])))])+1
